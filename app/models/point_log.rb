class PointLog < ApplicationRecord
  ...
  def self.aggregate_follow_user
    aggregate_id = ImportedUser.where(registered_on: nil).first.twitter_id
    return unless aggregate_id

    target_ids = PointLog.target_following_ids(aggregate_id)
    return unless target_ids

    PointLog.calculate_result(target_ids)
    FollowedUser.import_followed_users(target_ids)
    ImportedUser.find_by(twitter_id: aggregate_id).update(registered_on: Date.today)
  end

  def self.target_following_ids(confirm_user_id)
    # NOTE: 取得するユーザー数を絞っているのは負荷を減らすため。取得IDは小さい順や辞書順ではないため、それにより結果が偏ることはない。
    following_ids = twitter_rest_client.friend_ids(confirm_user_id.to_i).attrs[:ids].first(Settings.aggregate_param.get_follow_user_num)
